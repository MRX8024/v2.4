...
# DATA_FOLDER = 'C:/Users/nikit/Desktop/PycharmProjects/Shaper_graphs/raw_data_*.csv'
...
#     lognames = []
#
#     globbed_files = glob.glob(f'{DATA_FOLDER}/raw_data_*.csv')
...
def get_shaper_graph(accel_per_hz=None):
    current_date = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    # new_file = os.path.join(RESULTS_FOLDER, RESULTS_SUBFOLDERS[1], 'resonances_x_20240523_174211_adxl345.csv')
    ...
    accel_per_hz = accel_per_hz.split('=')[1]
    fig = shaper_calibration(new_file, KLIPPER_FOLDER, accel_per_hz)
    png_filename = os.path.join(RESULTS_FOLDER, RESULTS_SUBFOLDERS[1], f'resonances_{axis}_{current_date}_{accelerometer}_.png')
...
def main():
    ...
    if sys.argv[1].lower() == 'belts':
        ...
        fig, png_filename = get_shaper_graph(sys.argv[2])
    elif sys.argv[1].lower() == 'vibrations':
        fig, png_filename = get_vibrations_graph(sys.argv[2])
    else:
        print("Usage: plot_graphs.py [SHAPER|BELTS|VIBRATIONS]")
        sys.exit(1)
